// QR decode worker: the qrclaim-codec engine (a custom zxing-cpp build)
...
//
// Two decode paths (see ../../qrclaim-codec/wrapper/qrclaim_codec.cpp):
...
import QRClaimCodec, { type QRClaimModule, type QRClaimQuad } from "../vendor/qrclaim-codec/qrclaim_codec.js";

const ready: Promise<QRClaimModule> = QRClaimCodec({
...
function boundsOf(p: QRClaimQuad, ox: number, oy: number) {
...
function shifted(p: QRClaimQuad, ox: number, oy: number): QRClaimQuad {
...
ctx.onmessage = async (e: MessageEvent) => {
  ...
  try {
    ...
    const symbols: { bytes: Uint8Array; box: object; quad: QRClaimQuad; modules: number; tracked: boolean }[] = [];
