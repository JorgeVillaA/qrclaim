...
/**
 * Whether a landed transfer puts itself on screen. The only preference QRClaim
 ...
 */
const AUTO_SHOW_KEY = "qrclaim:auto-show";
...
// Default to everything the device offers. Workers got cheap (the QRClaim
...
/**
 * A QRClaim frame reached us, so the optical link works — retire the
...
function onDecoded(bytes: Uint8Array, box?: SymbolBox, info?: SymbolInfo) {
  ...
  if (done) return;
  if (!parsed) {
    // A QRClaim frame we cannot use is a different problem from no frames at
 ...
 * implying QRClaim had kept something it hadn't. Idempotent, because revealing
...
/** Deletes the received-media cache — the one thing QRClaim persists besides
