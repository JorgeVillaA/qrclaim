 ...
 *
 * The three source pages stay Spanish with data-i18n="dot.path" markers (see
 * shared/i18n/index.ts for the runtime half of this contract). At build time
 * this plugin:
 *
 *   - VERIFIES the inline Spanish against the es catalog and fails the build
...
/**
 * Check the inline Spanish against the es catalog. The HTML stays readable
 * Spanish on purpose; this is what keeps it from quietly diverging from what
 * the other eleven languages actually translate.
 */
function verifySourceInline(html: string, page: string, tokens: Record<string, string>): void {
  const fail = (path: string, inline: string, catalog: string): never => {
    throw new Error(
      `i18n-pages: ${page}: inline Spanish for "${path}" drifted from the catalog.\n` +
        `  inline:  ${inline}\n  catalog: ${catalog}\n` +
        `Edit both shared/i18n/locales/es.ts and the HTML — they are one copy.`,
    );
  };
  const source = CATALOGS[DEFAULT_LOCALE]!;
  for (const [, , , path, inner] of html.matchAll(TEXT_RE)) {
    const want = normalize(resolved(source, path!, tokens));
  ...
  for (const [tag, spec] of html.matchAll(ATTR_TAG_RE)) {
    for (const pair of spec!.split(";")) {
      ...
      const want = resolved(source, path, tokens);
...
function stampHtmlElement(html: string, locale: LocaleInfo): string {
  ...
  const sourceOpen = `<html lang="${CATALOGS[DEFAULT_LOCALE]!.meta.lang}">`;
  if (!html.includes(sourceOpen)) {
    throw new Error(`i18n-pages: expected the source page to open with ${sourceOpen}`);
  }
  return html.replace(sourceOpen, open);
}
  ...
  return {
    ...
    writeBundle(options, bundle) {
      ...
      for (const [fileName, pagePart] of Object.entries(PAGES)) {
        ...
        verifySourceInline(html, fileName, opts.tokens);

        // The default-locale (Spanish) pages get their own stamp, hreflang
        // set, and marker strip — written straight over the file vite just
        // emitted.
        let defaultPage = stampHtmlElement(html, LOCALES.find((l) => l.code === DEFAULT_LOCALE)!);
        defaultPage = insertBeforeHead(defaultPage, hreflangBlock(opts.siteUrl, pagePart));
        writeFileSync(resolve(dir, fileName), defaultPage.replace(STRIP_RE, ""));
