...
// script renders from. Unconfigured by default: set BENCHMARK_BADGE_URL to
// re-enable the shields.io badge.
const BADGE_URL = "";
...
function renderReadme(records: Records): void {
  ...
  if (entries.length === 0) {
    ...
    if (records.sustained && BADGE_URL) lines.push(`![sustained record](${BADGE_URL})`, "");
    lines.push(
