// Regenerates the PWA icon set in public/ from public/qrclaim_logo.svg:
//
...
const logo = readFileSync(join(publicDir, "qrclaim_logo.svg"), "utf8");
...
function squareVariant(markScale: number): string {
  for (const needle of [ROUNDED_RECT, MARK_OPEN]) {
    if (!cleaned.includes(needle)) {
      throw new Error(`qrclaim_logo.svg changed shape — expected to find: ${needle}`);
    }
  }
  return cleaned
...
const work = mkdtempSync(join(tmpdir(), "qrclaim-icons-"));
