 ...
 * as the AGPL requires, plus the notice for the MIT-licensed portions
 * the bundles incorporate (see NOTICE).
 *
...
export function licenseBanner(version: string): Plugin {
  const text =
    `QRClaim v${version} — SPDX-License-Identifier: AGPL-3.0-or-later`;
  const comment = `/*! ${text} */\n`;
