...
test("compressible files use gzip and recover exactly", async () => {
  const source = new TextEncoder().encode("qrclaim optical transfer\n".repeat(4_000));
...
test("gzip is still attempted for anything that might compress", () => {
  ...
  for (const type of [
    ...
    "application/vnd.qrclaim.snippet",
    "image/svg+xml",
    "image/bmp",
    "image/tiff",
    "image/x-icon",
...
test("a v3 receiver names a pre-versioning sender instead of going quiet", () => {
  ...
  for (const [marker, version] of [
    ...
    assert.match(frameVerdictMessage(verdict)!, /older QR Claim format/);
  }
});
...
test("a v3 receiver names a newer sender instead of going quiet", () => {
  ...
  assert.match(frameVerdictMessage(verdict)!, /newer QR Claim format/);
...
test("frames that are not QRClaim's stay silent", () => {
...
test("a stray 0xD1 alone never produces version advice", () => {
  ...
  // has never run QRClaim. Both magic bytes have to match before this file is
