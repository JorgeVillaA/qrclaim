...
//           keep their data-i18n attributes and Spanish text. Here the locale
...
const LOCALE_KEY = "qrclaim:locale";
const BANNER_DISMISSED_KEY = "qrclaim:locale-banner-dismissed";
const ISSUES_URL = "#";
 ...
 *
 *  A translated value with a %TOKEN% this runtime can't fill keeps its Spanish
...
export function applyHtmlTranslations(root: ParentNode, messages: Messages): void {
  const filled = (path: string): string | null => {
    ...
    if (/%[A-Z][A-Z0-9_]*%/.test(value)) {
      console.warn(`i18n: "${path}" needs a build-time token; keeping Spanish`);
      return null;
    }
    return value;
  };
...
export async function initI18n(): Promise<Messages> {
  ...
  const baked = staticLocale();
  if (!baked) {
    // Runtime path (standalone, dev): translate in place. Spanish needs no
