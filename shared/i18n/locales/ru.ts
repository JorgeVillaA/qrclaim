...
export const messages: Messages = {
  ...
  home: {
    title: "QRClaim Optical Transfer — передача файлов светом",
    metaDescription:
    ...
    shareSite: "Поделиться QRClaim",
    supportTitle: "Бесплатно, открытый код, без рекламы",
    supportBodyHtml:
      'Если QRClaim вам пригодился.',
  ...
  send: {
    docTitle: "QRClaim Optical Transfer — отправка",
    ...
    shareTitleData: "QRClaim Optical Transfer — приёмник",
  ...
  receive: {
    docTitle: "QRClaim Optical Transfer — приём",
    ...
    clearCache: "Очистить кэш QRClaim",
  ...
  verdicts: {
    olderSender: (version) =>
      `Передающий экран использует более старый формат QRClaim (v${version}). Обновите отправляющее устройство.`,
    newerSender: (version) =>
      `Передающий экран использует более новый формат QRClaim (v${version}). Обновите это приложение, чтобы принять поток.`,
    unsupportedFlags:
      "Этот поток использует возможность QRClaim, которую эта версия не умеет читать. Обновите это приложение, чтобы принять поток.",
  },
  ...
  i18n: {
    ...
    switchOffer: "QRClaim доступен на русском.",
