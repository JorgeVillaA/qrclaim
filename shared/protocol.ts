...
//   0  u8   magic 0xD1  ┐ together: "this is a QRClaim frame at all"
...
//     QRClaim — and that advice latches (see verdictShown in receive/main.ts),
 ...
 * can tell "older sender" from "not a QRClaim frame at all". These are magic1
...
export function classifyFrame(bytes: Uint8Array): FrameVerdict {
  ...
  // Past 16 bits of magic the frame is QRClaim's, so every verdict below can
...
export function frameVerdictMessage(verdict: FrameVerdict): string | null {
  switch (verdict.kind) {
    case "older-sender":
      return `That screen is sending an older QR Claim format (v${verdict.version}). Update the sending device.`;
    case "newer-sender":
      return `That screen is sending a newer QR Claim format (v${verdict.version}). Update this app to receive it.`;
    case "unsupported-flags":
      return "That stream uses a QR Claim feature this version cannot read. Update this app to receive it.";
    default:
      return null;
  }
}
